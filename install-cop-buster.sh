...
sudo mkdir /opt/cop/node_modules
sudo cp -r * /opt/cop/
sudo groupadd cop
...
sudo chown cop.cop /opt/cop/node_modules
...
if [ $ans -eq 1 ]; then
    ...
    echo "deb [ arch=amd64 ] https://repo.mongodb.org/apt/debian buster/mongodb-org/4.2 main" | sudo tee /etc/apt/sources.list.d/mongodb-org-4.2.list
    sudo apt-get update
